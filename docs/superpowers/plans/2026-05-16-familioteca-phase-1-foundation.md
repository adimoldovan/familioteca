...
This task has no automated checks — it's verification that the foundation works end-to-end.
...
- [ ] **Step 2: Seed and boot**

Run:
```bash
bin/rails db:reset   # wipes dev DB
bin/rails db:seed
bin/dev
```
...
1. Visit `http://localhost:3000/` → should redirect to `/sign_in`.
2. The form is in Romanian: "Autentificare", "Email", "Parolă", "Intră în cont".
3. Submit with wrong creds → see error "Email sau parolă greșite." (Romanian, no English).
4. Submit with `admin@familioteca.local` / `changeme123` → land on the empty catalog page showing "Bibliotecă" header and "Niciun titlu disponibil".
5. The header shows: app name, "Bibliotecă", "Administrare", "Administrator" name, "Deconectare" button.
6. Click "Administrare" → see members table (one row: Administrator).
7. Click "Deconectare" → redirected to `/sign_in`.
...
If there are stray files (e.g., `log/development.log` was tracked), add to `.gitignore` and commit.
